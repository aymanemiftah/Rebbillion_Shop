...
export default function ShowPayments(){
    ...
    const{payments=[],usersall}=useContext(DonnerContext)
    const{paymentsall=[]}=useContext(DonnerContext)
    ...
    return(
        ...
        <div className="bg-gray-100 min-h-screen rounded-lg p-8">
            ...
            <Titlepage role={'Admin'} page={'Payments'} />
            ...
            <CartInfo Title1={'Pending Payments'} Value1={paymentsall.filter((e) => e.status === 'pending').length}
              Title2={'Completed Payments'} Value2={paymentsall.filter((e) => e.status === 'completed').length}
               Title3={'Failed Payments'} Value3={paymentsall.filter((e) => e.status === 'failed').length}
               />
                ...
                <table className="w-full text-sm text-left rtl:text-right text-gray-500">
                    ...
                    <tbody className="divide-y divide-gray-200">
                        {payment && search !== "" ? payment.filter((payment) =>
                            ...
                            usersall.find((user) => user.id === payment.user_id)?.name.toLowerCase().startsWith(search.toLowerCase()) ||
                            ...
                            <tr key={key} className="hover:bg-gray-100 transition">
                                ...
                                <td className="px-6 py-4">{payment.user_id + " "}{usersall.find((e) => parseInt(e.id) === parseInt(payment.user_id))?.name}</td>
